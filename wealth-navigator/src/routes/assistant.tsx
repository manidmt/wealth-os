    ...
    ws.onmessage = (ev) => {
      ...
      if (typeof parsed?.token === "string") {
        ...
        if (!streamingIdRef.current) {
          ...
          setMessages((m) =>
            m.map((msg) => (msg.pending ? { ...msg, id, content: token, pending: false } : msg)),
          );
        } else {
          const id = streamingIdRef.current;
          setMessages((m) =>
            m.map((msg) =>
              msg.id === id ? { ...msg, content: msg.content + token, pending: false } : msg,
            ),
          );
        }
        return;
      ...
      if (parsed?.error) {
        setMessages((m) =>
          m.map((msg) =>
            msg.pending ? { ...msg, content: String(parsed!.error), pending: false } : msg,
          ),
        );
        streamingIdRef.current = null;
        setBusy(false);
    ...
    <AppShell pageEyebrow="Asistente">
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 py-8 md:px-8 lg:grid-cols-[1fr_280px]">
        ...
        <aside className="space-y-3">
          ...
          <div
            className={`rounded-lg border border-dashed p-3 text-[11px] leading-relaxed ${connected ? "border-emerald-500/40 bg-emerald-500/5 text-emerald-700 dark:text-emerald-400" : "border-border bg-muted/20 text-muted-foreground"}`}
          >
            <span
              className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${connected ? "bg-emerald-500" : "bg-muted-foreground/40"}`}
            />
            {connected
              ? "Agente conectado — respuestas reales con acceso a tus datos."
              : "Agente desconectado. Asegúrate de que el servidor está activo."}
          </div>
        </aside>
      </div>
    </AppShell>
      ...
      <div className="grid w-full max-w-xl gap-2 sm:grid-cols-2">
        {SUGGESTIONS.slice(0, 4).map((s) => (
          ...
          >
            ...
            <div className="mt-0.5 truncate text-[11px] text-muted-foreground">{s.prompt}</div>
          </button>
        ))}
      </div>
    </div>
